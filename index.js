...
const regexShifts = /<td class="right">(\d+)<\/td>/g

async function main(){
    message = "ERROR CHECK CODE"
    send = true
    let verificationTokenCookie, verificationTokenHTML;
    ...
    await fetch("https://app.betterimpact.com/Login/LoginNoSearch").then(async res => {
        ...
        verificationTokenHTML = await resText.match(/"__RequestVerificationToken" type="hidden" value="(.*?)"/)[1]
        console.log(verificationTokenCookie + " " + verificationTokenHTML)
    })

    
    fetch("https://app.betterimpact.com/Login/Login", {
        "headers": {
            ...
            "Referer": "https://app.betterimpact.com/Login/Login",
